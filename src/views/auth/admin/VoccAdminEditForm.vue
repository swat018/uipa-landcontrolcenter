...
const test = computed(() => {
  return voccAdminInfo.value.activated ? 'red' : 'yellow'
})
...
const getInfo = async () => {
  await voccStore.getVoccAdminInfo(props.voccAdminId)
    .then((response) => {
      ...
      voccAdminInfo.value.password = '********'
      toggle.value = voccAdminInfo.value.activated
    })
    .catch((error) => {
      console.error()
      vAlert('네트워크 오류')
...
const isActivated = computed(() => {
  return voccAdminInfo.value.activated ? '활성화' : '비활성화'
})
...
const changeAcitveStatus = () => {
  const userName = voccAdminInfo.value.username
  const activate = voccAdminInfo.value.activated
...
const removeAdmin = () => {
  ...
  const removeAdminInfo = {
    removeUserVoccName: voccAdminInfo.value.name,
