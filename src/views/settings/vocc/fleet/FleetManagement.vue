    ...
    <v-row class="h-100">
      <v-col cols="6">
        <v-card class="mt-4 h-100" rounded="30">
          ...
          <v-card-text>
            <DxDataGrid ref="fleetGrid" :data-source="fleets" key-expr="id" class="tab-container"
      ...
      <v-col cols="6">
        <v-card class="mt-4 h-100" rounded="30">
          ...
          <v-card-text>
            <DxDataGrid id="ships" ref="shipGrid" class="tab-container" key-expr="imoNumber" :data-source="initShips"
              :show-borders="true" :column-auto-width="true" :selected-row-keys="selectedRowKeys"
              @row-prepared="onRowPrepared" :show-column-lines="false" @selection-changed="onSelectionChanged"
              @editor-preparing="onEditorPreparing" :show-column-headers="false" :row-alternation-enabled="true">
...
const saveShipByFleet = () => {
  ...
  // 기존 선단에 없는 선박 배열
  // 추가된 선박 배열
  const newValue = selectedRowKeys.value.filter(x => !(oldShipKeys.includes(x)))
  // 삭제된 배열 
  const removedValue = oldShipKeys.filter(x => !(selectedRowKeys.value.includes(x)))
