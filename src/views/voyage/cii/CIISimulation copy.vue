<template>
  ...
  <v-sheet class="simulation-container detail-page d-flex flex-column h-100 pa-4">
    <v-row no-gutters class="">
      <v-col cols="6">
        <v-sheet class="d-flex ga-4">
          <div class="d-flex ga-2">
            <div class="datePicker"><input type="datetime-local" v-model="startDate" :max="yesterday" /></div>
            <div class="datePicker"><input type="datetime-local" v-model="endDate" :min="tomorrow" /></div>
      ...
      <v-col cols="6">
        <v-row no-gutters class="align-center">
          <v-col style="flex : 1 1 30%">
            <div class="d-flex ga-2">
              <div class="d-flex align-center">Parameter</div>
              <div class="datePicker"> <i-selectbox v-model="selectedParameter" :items="parameters"
                  variant="solo-filled" density="compact" class="equipmentSelector" bg-color="#434348"
                  placeholder="Parameter를 선택해주세요" :hide-details="true" width="20%"></i-selectbox></div>
            </div>
          </v-col>
          <v-col style="flex : 1 1 70%;">
            <v-sheet class="px-5">
              <div class="d-flex ga-2">
                <div class="d-flex align-center">CII Rating</div>
                <v-sheet color="#000000" style="flex : 1;">
                  <v-sheet class="cii-slider d-flex flex-column">
                    ...
                    <div class="d-flex gc-1 align-center" style="position: relative;">
                      ...
                      <v-sheet min-width="15%" height="12" color="#42D2A7" style=" ">
                      </v-sheet>
                      <v-sheet min-width="50%" height="12" color="#FEBD19" style="">
                      </v-sheet>
                      <v-sheet min-width="10%" height="12" color="#F68822" style="">
                      </v-sheet>
                      <v-sheet min-width="15%" height="12" color="#F04A4A"
                        style="border-top-right-radius : 20px;border-bottom-right-radius : 20px;">
                      </v-sheet>
                      <v-slider ref="ciirateSlider" id="ciirateSlider"
                        style="position: absolute; width: 100%; transform: translateY(-70%); top: 0%; height: 12px;"
                        min="40" max="160" v-model="ciiRate" thumb-label="always" @mouseup="simulateCiiData"></v-slider>
                    </div>

                    <div class="d-flex mt-1">
                      <div class="text-center" style="width : 10%; opacity : 0.5">A</div>
                      <div class="text-center" style="width : 15%">B</div>
    ...
    <!-- 테이블  -->
    <v-row class="row-height align-center mt-0 pa-2">
      <DxDataGrid id="cii-simulator-grid" ref="ciiSimulatorGrid" class="row-height" :data-source="pastCiiData.first"
        ...
        <DxColumn data-field="sailing" alignment="center" header-cell-template="sailing-header" :allow-editing="true" />
    ...
    <v-row class="row-height mt-0">
      <v-col cols="4" class="pa-2 h-100">
        <v-sheet color="#333334" class="h-100 pa-4">
          <div class="title">CII 예상 지표</div>
          <Echart :option="cimulationOption"></Echart>
        </v-sheet>
      </v-col>
      <v-col cols="8" class="pa-2 h-100">
        <v-sheet color="#333334" class="d-flex flex-column justify-space-between h-100 pa-4">
          <div class="title">CII 요약</div>
          <v-sheet color="#333334" class="summary-container d-flex flex-column">
            <div class="mb-2">현재 CII 지표 (2024-01-01 ~ 2024-04-29)</div>
            <div class="grade-container d-flex">
              <v-card class="grade-item d-flex">
                ...
                <v-card-item>
                  ...
                  <v-card-subtitle>
                    <span :class="getColorByGrade(pastCiiData.second.pastCiiGrade)"> {{ pastCiiData.second.pastCiiGrade
                      }}</span>
                  </v-card-subtitle>
                </v-card-item>
          ...
          <v-divider></v-divider>
          <v-sheet color="#333334" class="summary-container d-flex flex-column">
            <div class="mb-2">총 CII 지표 (2024-01-01 ~ 2024-12-31)</div>
            <div class="grade-container d-flex">
              <v-card class="grade-item d-flex">
                <template v-slot:prepend>
                  <div>
              ...
              <v-card class="grade-item d-flex">
                ...
                <v-card-item>
                  <v-card-title>운항거리</v-card-title>
                  <v-card-subtitle>
                    {{ pastCiiData.second.finalDistance }}<span class="unit"> nm</span>
                  </v-card-subtitle>
                </v-card-item>
              ...
              <v-card class="grade-item d-flex">
                ...
                <v-card-item>
                  <v-card-title>CO2 배출량</v-card-title>
                  <v-card-subtitle>
                    {{ pastCiiData.second.finalCo2Emission }}<span class="unit"> tCO₂</span>
                  </v-card-subtitle>
                </v-card-item>
...
const setChartOption = () =>{
  ...
  if (co2EmissionIndex == -1){
    ...
    series.value[co2EmissionIndex].data= [pastCiiData.value.first[0].co2Emission, pastCiiData.value.first[1].co2Emission]
  }
  ...
  if(attainedCiiIndex == -1){
    ...
    series.value[attainedCiiIndex].data = [pastCiiData.value.first[0].attainedCii, pastCiiData.value.first[1].attainedCii]
  }
}
