  ...
  <v-card class="h-100">
    ...
    <v-card-text class="title-form-container">
      <v-form @submit.prevent>
        ...
        <i-input type="text" v-model="VoccUserEditForm.voccName" :disabled='true'>
        </i-input>
        <div class="mt-4 mb-1">아이디</div>
        <i-input type="text" v-model="VoccUserEditForm.username" :disabled='true'>
        </i-input>
        ...
        <i-input type="text" v-model="VoccUserEditForm.nickname" placeholder="닉네임을 입력하여 주십시오" :disabled='true'>
        </i-input>
        <div class="mt-4 mb-1">이메일</div>
        <i-input type="text" v-model="VoccUserEditForm.email" placeholder="이메일을 입력하여 주십시오" :disabled='true'>
        </i-input>
      ...
      <AppModal v-model="isShowStautsModal" @close="closeStautsModal">
        ...
        <template #actions>
          <i-btnGroup type="confirm" @close="closeStautsModal" @confirm="changeAcitveStatusByAdmin"></i-btnGroup>
        </template>
      </AppModal>
 ...
 */
const getInfo = async () => {
  console.log('조회' + props.voccId, props.voccUserId)
  const response = await voccStore.getVoccUserInfo(props.voccId, props.voccUserId)
  VoccUserEditForm.value = response;
  VoccUserEditForm.value.password = '********'
  toggle.value = VoccUserEditForm.value.activated
}
...
const changeAcitveStatusByAdmin = () => {
  const userName = VoccUserEditForm.value.username
  const activate = VoccUserEditForm.value.activated
  const type = 'voccUser'

  adminStore.changeActiveByAdmin(userName, !activate, type).then(() => {
    isShowStautsModal.value = false;
  })
}
...
const removeVoccUser = () => {
  ...
  adminStore.removeVoccUser(voccUserId, removeVoccUserInfo).then(() => {
    isShowDeleteModal.value = false
    cancleChange()
  })
}
